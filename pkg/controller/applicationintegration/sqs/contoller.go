...
const (
	...
	errCreateQueueClient        = "cannot create Queue client"
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	if aws.BoolValue(p.Spec.UseServiceAccount) {
		...
		return &external{client: queueClient, kube: c.kube}, errors.Wrap(err, errCreateQueueClient)
	}
...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) { // nolint:gocyclo
	...
	sqs.LateInitialize(&cr.Spec.ForProvider, resAttributes.Attributes)
	...
	cr.Status.AtProvider = sqs.GenerateObservation(resAttributes.Attributes)
	if len(resAttributes.Attributes) != 0 {
...
func (e *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	if cr.Spec.ForProvider.FifoQueue && !strings.HasSuffix(meta.GetExternalName(cr), fifoQueueSuffix) {
		return managed.ExternalCreation{}, errors.New(errInvalidNameForFifoQueue)
	}

	res, err := e.client.CreateQueueRequest(sqs.GenerateCreateQueueInput(meta.GetExternalName(cr), &cr.Spec.ForProvider)).Send(ctx)
	if err != nil {
		return managed.ExternalCreation{}, errors.Wrap(err, errCreateFailed)
	}

	meta.SetExternalName(cr, *res.CreateQueueOutput.QueueUrl)
	if err := e.kube.Update(ctx, cr); err != nil {
...
func (e *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	_, err = e.client.TagQueueRequest(&awssqs.TagQueueInput{
		QueueUrl: aws.String(meta.GetExternalName(cr)),
		Tags:     sqs.GenerateQueueTags(&cr.Spec.ForProvider),
	}).Send(ctx)
	if err != nil {
		return managed.ExternalUpdate{}, errors.Wrap(err, errUpdateFailed)
	}
